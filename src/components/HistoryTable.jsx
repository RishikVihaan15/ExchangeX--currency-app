  ...
  if (!rows.length) {
    return (
      <div className="panel text-sm text-slate-500 dark:text-slate-400">
        No conversions yet. Head to the <span className="font-semibold text-ocean">Converter</span> to get started.
      </div>
    );
  }
            ...
            {rows.map((row, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-semibold">
                  {formatNumber(row.amount)} <span className="text-slate-400">{row.from}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="inline-flex items-center gap-1 rounded-lg bg-ocean/10 px-2 py-0.5 text-xs font-bold text-ocean dark:bg-ocean/20 dark:text-cyan-400">
                    {row.from} <FiArrowRight size={10} /> {row.to}
                  </span>
                </td>
                <td className="px-4 py-3 font-extrabold text-ocean dark:text-cyan-400 tabular-nums">
                  {formatCurrencyValue(row.result)} <span className="text-xs font-semibold text-slate-400">{row.to}</span>
                </td>
                <td className="px-4 py-3 text-slate-500 tabular-nums">{formatCurrencyValue(row.rate)}</td>
              </tr>
            ))}
          </tbody>
        </table>
